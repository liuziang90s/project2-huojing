...
<div>
        <div style="margin-bottom:10px;">
			...
			<form id="hyxt-do" class="form-inline" method="post" action="/HYTC/cq.do">
        ...
        <div>
			<table class="table table-bordered table-hover">
				...
				<tbody>
					<c:if test="${not empty requestScope.cq}">
						<c:forEach items="${requestScope.cq}" var="cq1">
							<tr>
								<c:forEach items="${cq1}" var="cq2"  varStatus="id"  begin="2" end="15">
								  <c:choose>
		                             <c:when test="${id.index==2 or id.index==7 or id.index==10}"><td style="display:none">${cq2}</td></c:when>
		                             <c:when test="${id.index==11}"><td style="display:none">${cq2}</td><td>${cq2=="0"?"无效":"有效"}</td></c:when>
		                             <c:otherwise><td>${cq2}</td></c:otherwise>
	                              </c:choose>
									
								</c:forEach>
								<td><a class="delete" href="javascript:void(0)"
									data-toggle="modal" data-target="#myModal">删除</a> <a
									class="weihu" href="javascript:void(0)">维护</a> <a
									class='huoyan' href="javascript:void(0)">火眼</a> <a
									class='video' href="javascript:void(0)">录像机</a></td>
							</tr>
						</c:forEach>
					</c:if>
				</tbody>
	...
	<!-- 火眼跳转 --> 
    <form style="display:none;" id="huoyan" method="get" action="/HYTC/hyxt.do">
        <input type="text" name="cqid" class="huoyan-id">
        <div class="">
	      <input class="register_input" type="hidden" name="forpage" value="web">
	   </div>
    </form>
    <!-- 录像机跳转 -->
    <form style="display:none;" id="video" method="get" action="/HYTC/lxj.do">
        <input type="text" name="cqid" class="video-id">
        <div class="">
	      <input class="register_input" name="forpage" value="web">
	   </div>
    </form>
...
<!--莫态框之删除-->
    ...
    <form id="user-update" action="/HYTC/param.do" method="post" style="display: none;">
        <div id="c5-set"></div>
        <div class="">
            <input class="register_input" name="forpage" value="cq-wh">
        </div>
    </form>
	...
	$(function(){
		...
		function page(){
			...
			if(num==sum){//当当前页为组后一页，禁用“下一页"按钮
	...
	//点击删除确认提交表单
	    $(function(){
	        $(".delete").click(function(){
	            ...
	            $("#del-form").attr("action", "/HYTC/cq.do");
	            $(".delete-p").text(txt);
	            $(".delete-id").attr("value", id);
	            $(".delete-id").attr("name", "cqid");
	        });
	...
	<script type="text/javascript">
	    $(function(){
	    	...
	    	$(".weihu").click(function(){
                ...
                var txt_arr=["cqid","cqname","name","people","phone","addid","dz","tel","userid","isvalid",""]
                for(var i=0;i<len;i++){
                    if(i !=10){
                    var txt=$(this).parent().siblings().eq(i).text();
                    	$("#c5-set").append("<input type='text' name="+txt_arr[i]+" value="+txt+">");
                    }
                }
                $("#user-update").submit();
            });	    	
